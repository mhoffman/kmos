...
subroutine run_proc_O2_ads_bridge_bridge(cell)
...
! Disable processes
    ...
    if(can_do(CO_ads_bridge,cell + (/ 0, 0, 0, 1/))) then
        call del_proc(CO_ads_bridge,cell + (/ 0, 0, 0, 1/))
    end if
...
! Update the lattice
    call replace_species(cell + (/0, 0, 0, pdo_bridge/),empty,O)
...
end subroutine run_proc_O2_ads_bridge_bridge
