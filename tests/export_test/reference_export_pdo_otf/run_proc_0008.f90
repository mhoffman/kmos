...
subroutine run_proc_CO_diff_bridge_hollow_l(cell)
...
! Disable processes
    ...
    if(can_do(O2_des_hollow_hollow_r,cell + (/ -2, 0, 0, 1/))) then
        call del_proc(O2_des_hollow_hollow_r,cell + (/ -2, 0, 0, 1/))
    end if
    ...
    if(can_do(O_diff_hollow_bridge_r,cell + (/ -1, 0, 0, 1/))) then
        call del_proc(O_diff_hollow_bridge_r,cell + (/ -1, 0, 0, 1/))
    end if
    if(can_do(O_diff_hollow_hollow_l,cell + (/ -1, 0, 0, 1/))) then
        call del_proc(O_diff_hollow_hollow_l,cell + (/ -1, 0, 0, 1/))
    end if
    ...
    if(can_do(O_diff_hollow_hollow_r,cell + (/ -1, 0, 0, 1/))) then
        call del_proc(O_diff_hollow_hollow_r,cell + (/ -1, 0, 0, 1/))
    end if
...
! Update the lattice
    ...
    call replace_species(cell + (/-1, 0, 0, pdo_hollow/),empty,CO)
...
! Enable processes
    ...
    call add_proc(CO_des_hollow, cell + (/ -1, 0, 0, 1/), gr_CO_des_hollow(cell + (/ -1, 0, 0, 0/)))
    ...
    case(O)
        call add_proc(CO_oxi_hollow_hollow_r, cell + (/ -2, 0, 0, 1/), gr_CO_oxi_hollow_hollow_r(cell + (/ -2, 0, 0, 0/)))
    case(empty)
        call add_proc(CO_diff_hollow_hollow_l, cell + (/ -1, 0, 0, 1/), gr_CO_diff_hollow_hollow_l(cell + (/ -1, 0, 0, 0/)))
    end select


end subroutine run_proc_CO_diff_bridge_hollow_l
