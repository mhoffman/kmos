...
class ModelBuilder(object):
    ...
    def pd100_build(self):
        ...
        new_sites = [
            ["bridge7", self.atoms.positions[[2, 14]].mean(axis=0)],
            ["bridge8", self.atoms.positions[[3, 14]].mean(axis=0)],
            [
                "bridge9",
                self.atoms.positions[[1, 14]].mean(axis=0) + (-self.y + self.x) / 2,
            ],
        ...
        for name, pos in new_sites:
            ...
            self.pt.layer_list[0].sites.append(
                Site(
                    name=name,
        ...
        self.coord_set = self.pt.layer_list.generate_coord_set(
            size=[3, 3, 1], layer_name=DEFAULT_LAYER
        )
        self.set_species()
        self.set_parameters()
    ...
    def export(self, filename):
        self.pt.export_xml_file(filename)
    ...
    def print_statistics(self):
        ...
        for process_type in [
            ...
            nprocs = len(
                [x for x in self.pt.get_processes() if x.name.startswith(process_type)]
            )
    ...
    def set_layers(self):
        ...
        sites = {}
        sites["bridge1"] = {
            "pos": frac(0.5 * (pos[2] + pos[1] - y) + z),
            "tags": "strongbridge CO",
        }
        sites["bridge2"] = {
            "pos": frac(0.5 * (pos[0] + pos[3]) + z),
            "tags": "strongbridge CO",
        }
        sites["bridge3"] = {
            "pos": frac(0.5 * (pos[0] + pos[2]) + z),
            "tags": "weakbridge CO",
        }
        sites["bridge4"] = {
            "pos": frac(0.5 * (pos[2] + pos[3] - x) + z),
            "tags": "weakbridge CO",
        }
        sites["bridge5"] = {
            "pos": frac(0.5 * (pos[3] + pos[1] + x) + z),
            "tags": "weakbridge CO",
        }
        sites["bridge6"] = {
            "pos": frac(0.5 * (pos[0] + pos[1]) + z),
            "tags": "weakbridge CO",
        }

        sites["side1"] = {
            "pos": frac((pos[2] + pos[0] - y) / 2.0 + z),
            "tags": "corner oxygen",
        }
        sites["side2"] = {
            "pos": frac((pos[2] + pos[3]) / 2.0 + z),
            "tags": "corner oxygen",
        }
        sites["side3"] = {
            "pos": frac((pos[3] + pos[1] - y + x) / 2.0 + z),
            "tags": "corner oxygen",
        }
        sites["side4"] = {
            "pos": frac((pos[0] + pos[1] + x) / 2.0 + z),
            "tags": "corner oxygen",
        }

        sites["hollow1"] = {
            "pos": frac((pos[0] + pos[2] + pos[1] + pos[3] - x) / 4.0 + z),
            "tags": "hollow oxygen",
        }
    ...
    def set_species(self):
        # Species
        self.pt.add_species(Species(name="empty", color="#ffffff", representation=""))
        self.pt.add_species(
            Species(
                name="CO",
                color="#000000",
    ...
    def set_parameters(self):
        self.pt.add_parameter(
            Parameter(name="T", value="600", adjustable=True, min="300", max="1500")
        )
        self.pt.add_parameter(
            Parameter(
                name="p_COgas",
    ...
    def set_co_adsorption_desorption(self):
        ...
        for i, coord in enumerate(
            [x for x in self.coord_set if "CO" in x.tags.split() and not any(x.offset)]
        ):
            blocked_coords = []
            ...
            for blocked_coord in blocked_coords:
                condition_list.append(
                    ConditionAction(coord=blocked_coord, species="empty")
                )
            action_list = [ConditionAction(coord=coord, species="CO")]
            proc = Process(
                name="CO_adsorption_%02i" % i,
        ...
        for initial_coord in self.coord_set:
            if not any(initial_coord.offset) and "CO" in initial_coord.tags:
                for final_coord in self.coord_set:
                    if (
                        "CO" in final_coord.tags
                        and 0
                        < np.linalg.norm(final_coord.pos - initial_coord.pos)
                        < 2.9
                    ):
                        final_blocked_sites = [
                            x
                            for x in self.coord_set
                            if 0 < np.linalg.norm(x.pos - final_coord.pos) < 2.9
                            and 0 < np.linalg.norm(initial_coord.pos - x.pos)
                        ]

                        conditions = [
                            ConditionAction(coord=initial_coord, species="CO"),
                            ConditionAction(coord=final_coord, species="empty"),
                        ]
                        conditions += [
                            ConditionAction(coord=blocked_site, species="empty")
                            for blocked_site in final_blocked_sites
                        ]

                        actions = [
                            ConditionAction(coord=initial_coord, species="empty"),
                        ...
                        rate_constant = (
                            "1/(beta*h)*exp(-beta*(E_CO_diff+max(0,%s-%s))*eV)"
                            % (E_final, E_initial)
                        )

                        self.pt.add_process(
                            Process(
                                name="CO_diffusion_%02i" % procs,
    ...
    def set_o2_adsorption_desorption(self):
        ...
        for i, (a, b) in enumerate(O2_pairs):
            ...
            # O2 adsorption
            condition_list = [
                ConditionAction(coord=coord_a, species="empty"),
                ConditionAction(coord=coord_b, species="empty"),
            ]
            action_list = [
                ConditionAction(coord=coord_a, species="O"),
            ...
            rate_constant = (
                "p_O2gas*bar*A*2/sqrt(2*pi*umass*m_O2/beta)*"
                + "exp(beta*(%s+%s-mu_O2gas)*eV)" % (E_a, E_b)
            )

            self.pt.add_process(
                Process(
                    name="O2_desorption_%02i" % i,
                    condition_list=condition_list,
                    action_list=action_list,
                    rate_constant=rate_constant,
                )
    ...
    def set_o_diffusion(self):
        ...
        for initial_coord in self.coord_set:
            if not any(initial_coord.offset) and "oxygen" in initial_coord.tags:
                for final_coord in self.coord_set:
                    if (
                        "oxygen" in final_coord.tags
                        and 0 < np.linalg.norm(final_coord.pos - initial_coord.pos) < 3
                    ):
                        final_blocked_sites = []
                        for final_blocked in self.coord_set:
                            if 0 < np.linalg.norm(
                                final_blocked.pos - final_coord.pos
                            ) < 2.9 and 0 < np.linalg.norm(
                                initial_coord.pos - final_blocked.pos
                            ):
                                final_blocked_sites.append(final_blocked)
                        conditions = []
                        conditions.append(
                            ConditionAction(coord=initial_coord, species="O")
                        )
                        conditions.append(
                            ConditionAction(coord=final_coord, species="empty")
                        )
                        ...
                        rate_constant = (
                            "1/(beta*h)*exp(-beta*(E_O_diff+max(0,%s-%s))*eV)"
                            % (E_final, E_initial)
                        )
                        self.pt.add_process(
                            Process(
                                name="O_diffusion_%02i" % procs,
                                condition_list=conditions,
                                action_list=actions,
                                rate_constant=rate_constant,
                            )
            ...
            if not any(O_coord.offset) and "oxygen" in O_coord.tags:
                for CO_coord in self.coord_set:
                    if (
                        ...
                        condition_list = [
                            ConditionAction(coord=O_coord, species="O"),
                            ConditionAction(coord=CO_coord, species="CO"),
                        ]

                        action_list = [
                            ConditionAction(coord=O_coord, species="empty"),
                            ConditionAction(coord=CO_coord, species="empty"),
                        ]

                        proc = Process(
                            name="Reaction_%02i" % len(procs),
                            condition_list=condition_list,
                            action_list=action_list,
