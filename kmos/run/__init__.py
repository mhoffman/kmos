...
class KMC_Model(Process):
    ...
    def reset(self):
        ...
        try:
            ...
            proclist.init(
                self.size, self.system_name, lattice.default_layer, not self.banner
            )
        ...
        if len(settings.lattice_representation):
            if hasattr(settings, "substrate_layer"):
                ...
                lattice_representation = eval(settings.lattice_representation)
                if len(lattice_representation) > 1:
                    self.lattice_representation = lattice_representation[
                        self.lattice.default_layer
                    ]
                else:
    ...
    def run(self):
        ...
        while True:
            ...
            if (
                self.autosend
                and self.image_queue is not None
                and not self.image_queue.full()
            ):
        ...
        for i in range(frames):
            ...
            if suffix == "png":
                ...
                for atom in atoms:
                    ...
                    colors += [
                        (
                            jmol_colors[atom.number][0],
                            jmol_colors[atom.number][1],
                            jmol_colors[atom.number][2],
                            0.00,
                            0.00,
                        )
                    ]
    ...
    def get_atoms(self, geometry=True, tag=None, reset_time_overrun=False):
                    ...
                    for k in range(lattice.system_size[2]):
                        for n in range(1, 1 + lattice.spuck):
                            ...
                            if self.species_representation.get(species, ""):
                                ...
                                if tag == "species":
                                    ad_atoms.set_initial_magnetic_moments(
                                        [species] * len(ad_atoms)
                                    )
                                elif tag == "site":
                                    ad_atoms.set_initial_magnetic_moments(
                                        [n] * len(ad_atoms)
        ...
        atoms.params = [
            float(self.settings.parameters.get(param_name)["value"])
            for param_name in sorted(self.settings.parameters)
            if self.settings.parameters[param_name].get("adjustable", False)
        ]
        ...
        if delta_steps == 0:
            ...
            atoms.tof_data = (
                self.tof_data
                if hasattr(self, "tof_data")
                else np.zeros_like(self.tof_matrix[:, 0])
            )
            atoms.tof_integ = (
                self.tof_integ
                if hasattr(self, "tof_integ")
                else np.zeros_like(self.tof_matrix[:, 0])
            )
        elif delta_t == 0.0 and atoms.kmc_time > 0 and reset_time_overrun:
            ...
            atoms.tof_data = np.dot(
                self.tof_matrix, (atoms.procstat - self.procstat) / delta_t / size
            )
            # S. Matera 09/25/2012
            if hasattr(self.base, "get_integ_rate"):
                atoms.tof_integ = np.dot(
                    self.tof_matrix,
    ...
    def get_std_sampled_data(
        ...
        for sample in range(samples):
            ...
            if tof_method == "procrates":
                ...
                raise NotImplementedError(
                    'tof_method="{tof_method}" not supported. Can be either procrates or integ.'.format(
                        **locals()
                    )
                )
        ...
        outdata = tuple(
            ...
            + [total_time, simulated_time, total_steps]
        )
        if output == "str":
    ...
    def switch_surface_processes_off(self):
        ...
        """
        for i, process_name in enumerate(sorted(self.settings.rate_constants)):
            if "diff" in process_name or "react" in process_name:
                self.base.set_rate_const(i + 1, 0.0)
    ...
    def print_adjustable_parameters(self, match=None, to_stdout=True):
        ...
        for i, attr in enumerate(sorted(self.settings.parameters)):
            if (match is None or fnmatch(attr, match)) and settings.parameters[attr][
                "adjustable"
            ]:
                res += "|{0:^78s}|\n".format(
                    (" %40s = %s" % (attr, settings.parameters[attr]["value"]))
                )
        res += (w * "-") + "\n"
        if to_stdout:
            print(res)
    ...
    def print_procstat(self, to_stdout=True):
        ...
        res += "+" + width * "-" + "+" + "\n"
        res += ("| {0:<%ss}|\n" % (width - 1)).format(
            "%9s %12s  %s" % ("rel. contrib.", "procstat", "process name")
        )
        res += "+" + width * "-" + "+" + "\n"
        for entry in entries:
            ...
            if procstat:
                res += ("|{0:<%ss}|\n" % width).format(
                    "%9.2f %% %12s     %s"
                    % (100 * float(printed_steps) / nsteps, procstat, name)
                )
    ...
    def print_state_summary(
        self, order="-rate", to_stdout=True, show=False, print_parameters=False
    ):
    ...
    def print_kmc_state(self, to_stdout=True):
        ...
        data_line = (
            "| kmc time {kmc_time:10.5g} | kmc steps {kmc_steps:18d} |\n".format(
                **locals()
            )
        )
        res = ("-" * len(data_line)) + "\n"
    ...
    def print_accum_rate_summation(self, order="-rate", to_stdout=True):
        ...
        if self.get_backend() in ["otf"]:
            res += "|{0:<118s}|\n".format(
                "(cumulative)    nrofsites,  rate                               [name]"
            )
        else:
            res += "|{0:<118s}|\n".format(
                "(cumulative)    nrofsites * rate_constant    = rate            [name]"
            )
        ...
        res += "+" + 118 * "-" + "+" + "\n"
        res += "|{0:<118s}|\n".format(("  = total rate = %.8e s^-1" % accum_rate))
        res += "+" + 118 * "-" + "+" + "\n"
    ...
    def _put(self, site, new_species, reduce=False):
        ...
        # Error checking
        if x not in range(self.lattice.system_size[0]):
            raise UserWarning("x-coordinate %s seems to fall outside lattice" % x)
        if y not in range(self.lattice.system_size[1]):
            raise UserWarning("y-coordinate %s seems to fall outside lattice" % y)
        if z not in range(self.lattice.system_size[2]):
            raise UserWarning("z-coordinate %s seems to fall outside lattice" % z)
        if n not in range(1, self.lattice.spuck + 1):
            raise UserWarning("n-coordinate %s seems to fall outside lattice" % n)
            ...
            for y in range(Y):
                for z in range(Z):
                    for n in range(N):
                        ...
                        # reduced onto one
                        choices = [
                            config[(x + i * X) % X, (y + j * Y) % Y, (z + k * Z) % Z, n]
                            for i in range(2)
                            for j in range(2)
                            for k in range(2)
                        ]
    ...
    def _get_configuration(self):
        ...
        """
        config = np.zeros(
            list(self.lattice.system_size) + [int(self.lattice.spuck)], dtype=np.int8
        )
        for x in range(self.lattice.system_size[0]):
            for y in range(self.lattice.system_size[1]):
                for z in range(self.lattice.system_size[2]):
                    for n in range(self.lattice.spuck):
                        config[x, y, z, n] = self.lattice.get_species([x, y, z, n + 1])
        return config
            ...
            for y in range(Y):
                for z in range(Z):
                    for n in range(N):
                        ...
                        self.lattice.replace_species(
                            [x, y, z, n + 1], species, config[x, y, z, n]
                        )
        self._adjust_database()
            ...
            for y in range(self.lattice.system_size[1]):
                for z in range(self.lattice.system_size[2]):
                    if self.get_backend() in ["lat_int", "otf"]:
                        eval("self.proclist.touchup_cell([%i, %i, %i, 0])" % (x, y, z))
                    else:
                        for n in range(self.lattice.spuck):
                            site_name = self.settings.site_names[n].lower()
                            eval(
                                "self.proclist.touchup_%s([%i, %i, %i, %i])"
                                % (site_name, x, y, z, n + 1)
                            )
    ...
    def rate_ratios(self, interactive=False):
        ratios = []
        for i, iname in enumerate(sorted(self.settings.rate_constants.keys())):
            for j, jname in enumerate(sorted(self.settings.rate_constants.keys())):
...
class Model_Parameters(object):
    ...
    def __setattr__(self, attr, value):
        if attr not in settings.parameters and attr not in ["print_rates"]:
    ...
    def __repr__(self):
        fixed_parameters = dict(
            (name, param)
            for name, param in settings.parameters.items()
...
class Model_Rate_Constants(object):
    ...
    def __call__(self, pattern=None, interactive=False, model=None):
        ...
        for i, proc in enumerate(sorted(settings.rate_constants.keys())):
            if pattern is None or fnmatch(proc, pattern):
                rate_expr = settings.rate_constants[proc][0]
                if model is None:
                    rate_const = evaluate_rate_expression(
                        rate_expr=rate_expr, parameters=settings.parameters
                    )
                else:
    ...
    def by_name(self, proc):
        ...
        """
        rate_expr = settings.rate_constants[proc][0]
        return evaluate_rate_expression(
            rate_expr=rate_expr, parameters=settings.parameters
        )
    ...
    def set(self, pattern, rate_constant, parameters=None):
        ...
        if type(rate_constant) is str:
            rate_constant = evaluate_rate_expression(
                rate_expr=rate_constant, parameters=parameters
            )
        try:
            rate_constant = float(rate_constant)
        except (ValueError, TypeError) as e:
            raise UserWarning(f"Could not convert rate constant to float: {e}")
        for i, proc in enumerate(sorted(settings.rate_constants.keys())):
            if pattern is None or fnmatch(proc, pattern):
...
class Model_Rate_Constants_OTF(Model_Rate_Constants):
    ...
    def _rate(self, procname, **kwargs):
        nr_vars = "".join(
            getattr(proclist_pars, "byst_{}".format(procname.lower()))
        ).split()
        if nr_vars:
            ...
            return getattr(proclist_pars, "rate_{}".format(procname.lower()))(
                input_array
            )
        else:
            return getattr(proclist_pars, "rate_{}".format(procname.lower()))()
    ...
    def bystanders(self, pattern=None, interactive=True):
        ...
        for i, proc in enumerate(sorted(settings.rate_constants.keys())):
            if pattern is None or fnmatch(proc, pattern):
                bysts = "".join(getattr(proclist_pars, "byst_{}".format(proc.lower())))
                res += "# %s: %s\n" % (proc, bysts)
        if interactive:
            print(res)
        else:
...
class ModelRunner(object):
    ...
    def __run_sublist(
        self, sublist, init_steps, sample_steps, samples, random_seed=None
    ):
        """
        ...
        for i, datapoint in enumerate(sublist):
            ...
            model = KMC_Model(
                ...
                cache_file="%s_configs/config_%s.pckl" % (self.runner_name, input_line),
            )
            ...
            data = model.get_std_sampled_data(
                samples=samples, sample_size=int(sample_steps), tof_method="integ"
            )
            ...
            if not os.path.exists(outfile):
                ...
                out.write(
                    """# If one or more parameters change between data lines\n# the set above corresponds to the first line.\n"""
                )
                out.close()
            out = open(outfile, "a")
            out.write(data)
    ...
    def plot(
        ...
        for suffix in suffixes:
            if label is None:
                if sublabel is None:
                    plt.savefig(
                        "%s_coverages.%s" % (self.runner_name, suffix),
                        bbox_inces="tight",
                    )
                else:
                    plt.savefig(
                        "%s_%s_coverages.%s" % (self.runner_name, sublabel, suffix),
                        bbox_inces="tight",
                    )
            else:
                if sublabel is None:
                    ...
                    plt.savefig(
                        "%s_%s_coverages.%s" % (label, sublabel, suffix),
                        bbox_inces="tight",
                    )
        ...
        for suffix in suffixes:
            if label is None:
                plt.savefig(
                    "%s_TOFs.%s" % (self.runner_name, suffix), bbox_inches="tight"
                )
            else:
    ...
    def run(
        self, init_steps=1e8, sample_steps=1e8, cores=4, samples=1, random_seed=None
    ):
...
def set_rate_constants(parameters=None, print_rates=None):
    ...
    for proc in sorted(settings.rate_constants):
        rate_expr = settings.rate_constants[proc][0]
        rate_const = evaluate_rate_expression(
            rate_expr=rate_expr, parameters=parameters
        )

        if rate_const < 0.0:
            raise UserWarning(
                "%s = %s: Negative rate-constants do no make sense"
                % (rate_expr, rate_const)
            )
        try:
            base.set_rate_const(getattr(proclist, proc.lower()), rate_const)
            if print_rates:
        ...
        for name, entry in settings.parameters.items():
            proclist.update_user_parameter(
                ...
                evaluate_rate_expression(
                    rate_expr=str(entry["value"]).split(" ")[
                        0
                    ],  # FIXME Take first item of lists to support deprecated 'lattice_size' parameter
    ...
    if hasattr(proclist, "update_chempot"):
        for chempot in settings.chemical_potentials:
            proclist.update_chempot(
                ...
                evaluate_rate_expression(rate_expr=chempot, parameters=parameters),
            )
