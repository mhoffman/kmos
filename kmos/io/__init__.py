    ...
    for key, value in dictionary.items():
        if isinstance(value, dict):
            if isinstance(key, Coord):
                out.write(
                    "%sselect case(get_species(cell%s))\n" % (indent, key.radd_ff())
                )
...
class ProcListWriter:
    ...
    def write_template(self, filename, target=None, options=None):
        ...
        with open(
            os.path.join(self.dir, "{target}.f90".format(**locals())), "w"
        ) as out:
            out.write(
                evaluate_template(template, self=self, data=self.data, options=options)
            )
    ...
    def write_proclist(self, smart=True, code_generator="local_smart"):
        ...
        if code_generator == "local_smart":
            ...
            constants_out.write(
                evaluate_template(
                    template, self=self, data=data, module_name="proclist_constants"
                )
            )
            constants_out.close()
            parameters_out = open("%s/proclist_pars.f90" % self.dir, "w")
            self.write_proclist_pars_otf(
                data, parameters_out, separate_files=self.separate_proclist_pars
            )
    ...
    def write_proclist_generic_part(self, data, out, code_generator="local_smart"):
        ...
        out.write("\n\ncontains\n\n")
        self.write_proclist_generic_subroutines(
            data, out, code_generator=code_generator
        )

    def write_proclist_generic_subroutines(
        self, data, out, code_generator="local_smart"
    ):
        from kmos.utils import evaluate_template
    ...
    def write_proclist_generic_subroutines_acf(
        self, data, out, code_generator="local_smart"
    ):
        from kmos.utils import evaluate_template
        ...
        for process in data.process_list:
            ...
            for condition in process.condition_list:
                corresponding_actions = [
                    action for action in actions if condition.coord == action.coord
                ]

                self._db_print(
                    "%s: %s <-> %s" % (process.name, condition, corresponding_actions)
                )
            ...
            for action in actions:
                if action not in true_actions:
                    if not (
                        action.species.startswith("^") or action.species.startswith("$")
                    ):
                        # raise UserWarning('Found unmatched action that is not a multi-lattice action: %s' % action)
                        print(
                            (
                                "UserWarning: Found unmatched action (%s) that is not a multi-lattice action: %s"
                                % (process.name, action)
                            )
                        )
    ...
    def write_proclist_lat_int(self, data, out, debug=False):
        ...
        representation_length = max(
            [len(species.representation) for species in data.species_list]
        )
        out.write(
    ...
    def write_proclist_lat_int_run_proc_nr(
        ...
        for lat_int_loop, (lat_int_group, processes) in enumerate(
            ...
            for action in process0.action_list:
                ...
                for _, other_processes in lat_int_groups.items():
                    ...
                    self._db_print(
                        "      OTHER PROCESS %s" % (pformat(other_process, indent=12))
                    )
                    other_conditions = (
                        other_process.condition_list + other_process.bystanders
                    )
                    self._db_print(
                        "            OTHER CONDITIONS\n%s"
                        % pformat(other_conditions, indent=12)
                    )
                    ...
                    for condition in other_conditions:
                        if action.coord.eq_mod_offset(condition.coord):
                            modified_procs.add(
                                (
                                    other_process,
                                    tuple(action.coord.offset - condition.coord.offset),
                                )
                            )

            # sort to one well-defined orded
            modified_procs = sorted(
                modified_procs, key=lambda x: "%s %s" % (x[0].name, str(x[1]))
            )
            ...
            for i, (process, offset) in enumerate(modified_procs):
                ...
                out.write(
                    "    call del_proc(nli_%s(cell + %s), cell + %s)\n"
            ...
            for condition in process0.condition_list:
                try:
                    action = [
                        action
                        for action in process0.action_list
                        if condition.coord == action.coord
                    ][0]
            ...
            for action in process0.action_list:
                if action not in matched_actions:
                    ...
                    if action.species.startswith("$"):
                        ...
                        raise UserWarning(
                            "Unmatched action that is not a multi-lattice action: %s"
                            % (action)
                        )
            ...
            for i, (process, offset) in enumerate(modified_procs):
                ...
                offset_site = "(/%+i, %+i, %+i, 1/)" % tuple(offset)
                out.write(
                    "    call add_proc(nli_%s(cell + %s), cell + %s)\n"
                    % (process.name, offset_cell, offset_site)
                )
            ...
            if os.name == "posix":
                progress_bar.render(
                    int(10 + 40 * float(lat_int_loop) / len(lat_int_groups)),
                    ...
                    for op in ["put", "take"]:
                        ...
                        if op == "put":
                            if data.meta.debug > 0:
                                ...
                                out.write(
                                    'print *,"    LATTICE/REPLACE_SPECIES/OLD_SPECIES","%s"\n'
                                    % data.species_list.default_species
                                )
                                out.write(
                                    'print *,"    LATTICE/REPLACE_SPECIES/NEW_SPECIES","%s"\n'
                                    % species.name
                                )
                            out.write(
                                "    call replace_species(site, %s, %s)\n\n"
                        ...
                        for process in data.process_list:
                            for condition in process.condition_list:
                                if (
                                    ...
                                    and layer.name == condition.coord.layer
                                ):
                                    ...
                                    if (
                                        ...
                                        other_conditions = list(
                                            filter(
                                                lambda x: x.coord != condition.coord,
                                                process.condition_list,
                                            )
                                        )
                                    ...
                                    # something else
                                        ...
                                        coord = (
                                            process.executing_coord() - condition.coord
                                        )
                        ...
                        if disabled_procs:
                            ...
                            for process, coord in disabled_procs:
                                if data.meta.debug > 1:
                                    out.write(
                                        'print *,"    LATTICE/CAN_DO/PROC",%s\n'
                                        % process.name
                                    )
                                    out.write(
                                        'print *,"    LATTICE/CAN_DO/VSITE","site%s"\n'
                                        % (coord).radd_ff()
                                    )
                                    out.write(
                                        'print *,"    LATTICE/CAN_DO/SITE",site%s\n'
                                        % (coord).radd_ff()
                                    )
                                # out.write(('    if(can_do(%(proc)s, site%(coord)s))then\n'
                                out.write(
                                    (
                                        "    if(avail_sites(%(proc)s, lattice2nr(%(unpacked)s), 2).ne.0)then\n"
                        ...
                        if enabled_procs:
                            ...
                            self._write_optimal_iftree(
                                items=enabled_procs, indent=4, out=out
                            )
                        out.write("\nend subroutine %s\n\n" % routine_name)
        ...
        for layer in data.layer_list:
            for site in layer.sites:
                ...
                for process in data.process_list:
                    ...
                    if (
                        executing_coord.layer == layer.name
                        ...
                        condition_list = [
                            ConditionAction(
                                ...
                                coord="site%s"
                                % (condition.coord - executing_coord).radd_ff(),
                            )
                            for condition in process.condition_list
                        ]
                    ...
                    for special_op in ["create", "annihilate"]:
                        ...
                        if special_op == "create":
                            if data.meta.debug > 0:
                                ...
                                out.write(
                                    'print *,"    LATTICE/REPLACE_SPECIES/OLD_SPECIES","null_species"\n'
                                )
                                out.write(
                                    'print *,"    LATTICE/REPLACE_SPECIES/NEW_SPECIES",species\n'
                                )
                            out.write(
                            ...
                            if data.meta.debug > 0:
                                ...
                                out.write(
                                    'print *,"    LATTICE/REPLACE_SPECIES/OLD_SPECIES",species\n'
                                )
                                out.write(
                                    'print *,"    LATTICE/REPLACE_SPECIES/NEW_SPECIES","null_species"\n'
                                )
                            out.write(
                        ...
                        for process in data.process_list:
                            for condition in filter(
                                lambda condition: condition.coord.name == site.name
                                and condition.coord.layer == layer.name,
                                process.condition_list,
                            ):
                                if special_op == "create":
                                    other_conditions = [
                        ...
                        if disabled_procs:
                            ...
                            for process, coord in disabled_procs:
                                ...
                                out.write(
                                    (
                                        "    if(can_do(%(proc)s, site%(coord)s))then\n"
                                        + "        call del_proc(%(proc)s, site%(coord)s)\n"
                                        + "    endif\n\n"
                                    )
    ...
    def _write_optimal_iftree(self, items, indent, out):
        ...
        most_common_coord = _most_common(
            [y.coord for y in _flatten([x[0] for x in items])]
        )
        ...
        answers = [
            y.species
            for y in filter(
                lambda x: x.coord == most_common_coord, _flatten([x[0] for x in items])
            )
        ]
        ...
        if self.data.meta.debug > 1:
            out.write(
                'print *,"    LATTICE/GET_SPECIES/VSITE","%s"\n' % most_common_coord
            )
            out.write('print *,"    LATTICE/GET_SPECIES/SITE",%s\n' % most_common_coord)
            out.write(
                'print *,"    LATTICE/GET_SPECIES/SPECIES",get_species(%s)\n'
                % most_common_coord
            )

        out.write(
            "%sselect case(get_species(%s))\n" % ((indent) * " ", most_common_coord)
        )
        for answer in uniq_answers:
            ...
            nested_items = list(
                filter(
                    lambda x: (
                        ...
                        and answer
                        == list(filter(lambda y: y.coord == most_common_coord, x[0]))[
                            0
                        ].species
                    ),
                    items,
            ...
            for nested_item in nested_items:
                conditions = list(
                    filter(lambda x: most_common_coord != x.coord, nested_item[0])
                )
    ...
    def write_proclist_pars_otf(self, data, out, separate_files=False):
        ...
        for ip, parameter in enumerate(
            sorted(data.parameter_list, key=lambda x: x.name)
        ):
            out.write(
                "integer(kind=iint), public :: %s = %s\n" % (parameter.name, (ip + 1))
            )
        out.write(
            "real(kind=rdouble), public, dimension(%s) :: userpar\n"
        ...
        for const in units_list:
            out.write(
                "real(kind=rdouble), parameter :: %s = %.10e\n"
                % (const, evaluate_rate_expression(rate_expr=const))
            )
        out.write("\n! Species masses\n")
        for mass in masses_list:
            out.write(
                "real(kind=rdouble), parameter :: %s = %.10e\n"
                % (mass, evaluate_rate_expression(rate_expr=mass))
            )
        ...
        if chempot_list:
            ...
            out.write(
                "real(kind=rdouble), public, dimension(%s) :: chempots\n"
                % len(chempot_list)
            )
        ...
        for iproc, process in enumerate(data.get_processes()):
            ...
            nr_vars = sorted(nr_vars, key=lambda x: (x.split("_")[2], x.split("_")[1]))
            ...
            out2.write(
                'character(len={0}), parameter, public :: byst_{1} = "{2}"\n'.format(
                    nr_vars_str_len, process.name, nr_vars_print
                )
            )
            ...
            after_contains2 = after_contains2 + (
                "%sinteger(kind=iint), dimension(4), intent(in) :: cell\n"
                % (" " * indent)
            )
            if nr_vars:
                after_contains2 = after_contains2 + (
            ...
            if aux_vars:
                after_contains2 = after_contains2 + (
                    "! Process specific auxiliary variables\n"
                )
                for aux_var in aux_vars:
                    after_contains2 = after_contains2 + (
                        "%sreal(kind=rdouble) :: %s\n" % (" " * indent, aux_var)
                    )
                after_contains2 = after_contains2 + ("\n")

            after_contains2 = after_contains2 + (
                "{0}real(kind=rdouble) :: rate_{1}\n".format(" " * indent, process.name)
            )

            # Update the value of the rate expression to account for the nr_var array
            for iv, nr_var in enumerate(nr_vars):
                new_expr = new_expr.replace(nr_var, "nr_vars({0:d})".format(iv + 1))
            ## TODO Merge this into the parser function
            new_expr = new_expr.replace(
                "gr_{0}".format(process.name), "rate_{0}".format(process.name)
            )
        ...
        for process in data.process_list:
            ...
            for expr in exprs:
                ...
                try:
                    ...
                    raise Exception(f"Could not tokenize expression: {expr}") from e
        ...
        if expr:
            ...
            if len(rate_lines) == 1:
                if "=" not in rate_lines[0]:
                    ...
                    raise ValueError(
                        "Bad expression for single line otf rate\n"
                        + "{}\n".format(rate_lines[0])
                        + " must assign value to 'otf_rate'"
                    )
            elif "otf_rate" not in expr:
                raise ValueError(
                    "Found a multiline otf_rate expression without 'otf_rate' on it"
                )
            final_expr = ""
            for rate_line in rate_lines:
                if "=" in rate_line:
                    ...
                    if (
                        not aux_var == "otf_rate"
                        and not aux_var.startswith("nr_")
                        and aux_var not in aux_vars
                    ):
    ...
    def _parse_otf_rate_line(self, expr, procname, data, indent=4):
        ...
        try:
            ...
            raise Exception(f"kmos.io: Could not tokenize expression: {expr}") from e
        replaced_tokens = []
        split_expression = ""
        currl = 0
        for i, token, _, _, _ in tokens:
    ...
    def write_proclist_touchup_otf(self, data, out):
        ...
        for process in data.process_list:
            rel_pos = (
                ...
                0,
                0,
            )  # during touchup we only activate procs from current site
            ...
            enabling_items.append(
                (copy.deepcopy(process.condition_list), copy.deepcopy(item2))
            )
    ...
    def write_proclist_run_proc_name_otf(
        self, data, out=None, separate_files=False, indent=4
    ):
        ...
        for iproc, exec_proc in enumerate(data.get_processes()):
            ...
            for exec_action in exec_proc.action_list:
                ...
                for ip, proc in enumerate(process_list):
                    for condition in proc.condition_list:
                        if (
                            condition.coord.name == exec_action.coord.name
                            and condition.coord.layer == exec_action.coord.layer
                        ):
                            ...
                            rel_pos = tuple(
                                (exec_action.coord - condition.coord).offset
                            )
                    ...
                    for byst in proc.bystander_list:
                        if (
                            byst.coord.name == exec_action.coord.name
                            and byst.coord.layer == exec_action.coord.layer
                        ):
            ...
            for ip in range(nprocs):
                enh_procs[ip] = [
                    rel_pos
                    for rel_pos in set(enh_procs[ip])
                    if rel_pos not in inh_procs[ip]
                ]
                aff_procs[ip] = [
                    rel_pos
                    for rel_pos in set(aff_procs[ip])
                    if rel_pos not in inh_procs[ip]
                ]
            ...
            for ip, sublist in enumerate(inh_procs):
                for rel_pos in sublist:
                    out2.write(
                        "%sif(can_do(%s,cell + (/ %s, %s, %s, 1/))) then\n"
                        % (
                            " " * indent,
            ...
            for exec_action in exec_proc.action_list:
                ...
                matching_conds = [
                    cond
                    for cond in exec_proc.condition_list
                ...
                if len(matching_conds) == 1:
                    ...
                    raise RuntimeError(
                        "Found wrong number of matching conditions: %s"
                        % len(matching_conds)
                    )
                out2.write(
                    "%scall replace_species(cell%s,%s,%s)\n"
                    % (
                        " " * indent,
            ...
            for ip, sublist in enumerate(enh_procs):
                for rel_pos in sublist:
                    ...
                    for cond in process_list[ip].condition_list:
                        ...
                        for exec_action in exec_proc.action_list:
                            if (
                                exec_action.coord.name == cond.coord.name
                                and exec_action.coord.layer == cond.coord.layer
                                and rel_pos
                                == tuple((exec_action.coord - cond.coord).offset)
                            ):
                                if not exec_action.species == cond.species:
                                    raise RuntimeError(
                                        "Found discrepancy in process selected for enabling!"
                                    )
                                else:
                                    break
                        else:
                            relative_coord = Coord(
                                name=cond.coord.name,
                                layer=cond.coord.layer,
                                offset=cond.coord.offset + np.array(rel_pos),
                            )
                            other_conditions.append(
                                ConditionAction(
                                    coord=relative_coord, species=cond.species
                                )
                            )
                    enabling_items.append(
                        (copy.deepcopy(other_conditions), copy.deepcopy(item2))
                    )
    ...
    def _write_optimal_iftree_otf(self, items, indent, out):
        ...
        for item in filter(lambda x: not x[0], items):
            ...
            # disabling (=False) a process
            if item[1][2]:
                rel_cell = "cell + (/ %s, %s, %s, 0/)" % (
                    item[1][1][0],
                    item[1][1][1],
                    item[1][1][2],
                )
                rel_site = "cell + (/ %s, %s, %s, 1/)" % (
                    item[1][1][0],
                    item[1][1][1],
                    item[1][1][2],
                )
                out.write(
                    "%scall add_proc(%s, %s, gr_%s(%s))\n"
                    % (" " * indent, item[1][0], rel_site, item[1][0], rel_cell)
                )
            else:
                out.write(
        ...
        if self.data.meta.debug > 1:
            out.write(
                'print *,"    IFTREE/GET_SPECIES/VSITE","%s"\n' % most_common_coord
            )
            out.write(
                'print *,"    IFTREE/GET_SPECIES/SITE","%s"\n'
    ...
    def write_settings(self, code_generator="lat_int"):
        ...
        for process in data.process_list:
            ...
            try:
                ...
                raise UserWarning(
                    "Parameter ill-defined(%s)\n%s\nProcess: %s"
                    % (param, e, process.name)
                )

            try:
                evaluate_rate_expression(
                    rate_expr=process.rate_constant,
        ...
        for species in sorted(data.get_speciess(), key=lambda x: x.name):
            out.write(
                '    "%s":"""%s""",\n' % (species.name, species.representation.strip())
            )
        out.write("    }\n\n")
        out.write(
            'lattice_representation = """%s"""\n\n' % data.layer_list.representation
        )

        # Species Tags
        out.write("species_tags = {\n")
        for species in sorted(data.get_speciess(), key=lambda x: x.name):
            out.write('    "%s":"""%s""",\n' % (species.name, species.tags.strip()))
        out.write("    }\n\n")
...
def export_source(
    ...
    if code_generator == "local_smart":
        cp_files = [
            (os.path.join("fortran_src", "assert.ppc"), "assert.ppc"),
    ...
    for filename, target in cp_files:
        shutil.copy(
            os.path.join(APP_ABS_PATH, filename), os.path.join(export_dir, target)
        )
