...
class Project(object):
    ...
    def __init__(self):
        ...
        self.get_layers = lambda: sorted(self.layer_list, key=lambda x: x.name)

        self.add_output = lambda output: self.output_list.append(output)
        self.get_outputs = lambda: sorted(self.output_list, key=lambda x: x.name)
    ...
    def add_species(self, *speciess, **kwargs):
        ...
        # do it now!
        if len(self.species_list) == 1 and not hasattr(
    ...
    def add_site(self, **kwargs):
        ...
        try:
            layer = [layer for layer in self.get_layers() if layer.name == layer_name][
                0
            ]
        except IndexError:
    ...
    def _get_ini_string(self):
        ...
        if hasattr(self.species_list, "default_species"):
            config.set(
                "SpeciesList", "default_species", self.species_list.default_species
            )
        else:
        ...
        for species in self.get_speciess():
            ...
            if hasattr(species, "representation"):
                config.set(section_name, "representation", species.representation)
        ...
        if hasattr(self.layer_list, "cell"):
            config.set(
                "Lattice",
                "cell_size",
                " ".join([str(i) for i in self.layer_list.cell.flatten()]),
            )
            ...
            if hasattr(self.layer_list, "substrate_layer"):
                config.set(
                    "Lattice", "substrate_layer", self.layer_list.substrate_layer
                )

        if hasattr(self.layer_list, "representation"):
            config.set("Lattice", "representation", self.layer_list.representation)
        ...
        for layer in self.get_layers():
            ...
            for site in layer.sites:
                ...
                pos = (
                    tuple(site.pos.tolist())
                    if hasattr(site.pos, "tolist")
                    else tuple(site.pos)
                )
                config.set(
                    section_name,
        ...
        for process in self.get_processes():
            ...
            config.set(
                section_name,
                "otf_rate",
                str(process.otf_rate) if process.otf_rate is not None else "None",
            )
            config.set(section_name, "enabled", str(process.enabled))
            if process.bystander_list:
                bystanders = [
                    bystander._shorthand() for bystander in process.bystander_list
                ]
                print(process.name)
                print(bystanders)
            ...
            conditions = [
                condition._shorthand() for condition in process.condition_list
            ]
            config.set(section_name, "conditions", " + ".join(conditions))

            actions = [action._shorthand() for action in process.action_list]
            config.set(section_name, "actions", " + ".join(actions))
    ...
    def _get_etree_xml(self):
        ...
        if hasattr(self.species_list, "default_species"):
            species_list.set("default_species", self.species_list.default_species)
        else:
        ...
        if hasattr(self.layer_list, "cell"):
            lattice_elem.set(
                "cell_size", " ".join([str(i) for i in self.layer_list.cell.flatten()])
            )
            if hasattr(self.layer_list, "default_layer"):
                lattice_elem.set("default_layer", self.layer_list.default_layer)
            if hasattr(self.layer_list, "substrate_layer"):
                lattice_elem.set("substrate_layer", self.layer_list.substrate_layer)
        if hasattr(self.layer_list, "representation"):
        ...
        for process in self.get_processes():
            ...
            for condition in process.condition_list:
                ...
                condition_elem.set(
                    "coord_offset", " ".join([str(i) for i in condition.coord.offset])
                )
            for action in process.action_list:
                ...
                action_elem.set(
                    "coord_offset", " ".join([str(i) for i in action.coord.offset])
                )
    ...
    def shorten_names(self, max_length=15):
        ...
        with open(
            "abbreviations_{self.meta.model_name}.dat".format(**locals()), "w"
        ) as outfile:
    ...
    def save(self, filename=None, validate=True):
        ...
        if filename.endswith(".xml"):
            ...
            raise UserWarning(
                "Cannot export to file suffix %s" % os.path.splitext(filename)[-1]
            )
        ...
        for section in config.sections():
            if section == "Lattice":
                ...
                for option in options:
                    ...
                    if option == "cell_size":
                        cell = np.array([float(i) for i in value.split()])
                        if len(cell) == 3:
                ...
                if "color" in options:
                    layer = self.add_layer(
                        Layer(name=layer_name, color=config.get(section, "color"))
                    )
                else:
                ...
                min = config.getfloat(section, "min") if "min" in options else 0.0
                max = config.getfloat(section, "max") if "max" in options else 0.0
                ...
                adjustable = (
                    config.getboolean(section, "adjustable")
                    if "adjustable" in options
                    else None
                )
                self.add_parameter(
                    Parameter(
                ...
                for action in [
                    x.strip() for x in config.get(section, "actions").split("+")
                ]:
                    try:
                        ...
                        print(action.split("@"))
                        raise ValueError(
                            f"Failed to parse action '{action}': {e}"
                        ) from e
                    coord = coord.split(".")
                    if len(coord) == 3:
                ...
                for condition in [
                    x.strip() for x in config.get(section, "conditions").split("+")
                ]:
                ...
                if "bystanders" in config.options(section):
                    for bystander in [
                        x.strip() for x in config.get(section, "bystanders").split("+")
                    ]:
                ...
                name = section.split()[-1]
                options = config.options(section)
                color = config.get(section, "color") if "color" in options else ""
                representation = (
                    config.get(section, "representation")
                    if "representation" in options
                    else ""
                )
                tags = config.get(section, "tags") if "tags" in options else ""
                self.add_species(
                    Species(
                        name=name, color=color, representation=representation, tags=tags
                    )
                )
    ...
    def import_xml_file(self, filename):
        ...
        if os.path.exists(filename):
            try:
                ...
                raise Exception(
                    (
                        "Could not parse file %s. Are you sure this"
        ...
        if self.version not in supported_versions:
            ...
            for child in root:
                if child.tag == "lattice":
                    cell = np.array(
                        [float(i) for i in child.attrib["cell_size"].split()]
                    )
                    if len(cell) == 3:
                        self.layer_list.cell = np.diag(cell)
                    elif len(cell) == 9:
                    ...
                    if "substrate_layer" in child.attrib:
                        self.layer_list.substrate_layer = child.attrib[
                            "substrate_layer"
                        ]
                    else:
                        self.layer_list.substrate_layer = self.layer_list.default_layer
                    if "representation" in child.attrib:
                        self.layer_list.representation = child.attrib["representation"]
                    else:
                    ...
                    for parameter in child:
                        ...
                        min = (
                            float(parameter.attrib["min"])
                            if "min" in parameter.attrib
                            else 0.0
                        )
                        max = (
                            float(parameter.attrib["max"])
                            if "max" in parameter.attrib
                            else 0.0
                        )
                        scale = (
                            parameter.attrib["scale"]
                            if "scale" in parameter.attrib
                            else "linear"
                        )
                    ...
                    for process in child:
                        ...
                        for sub in process:
                            ...
                            if sub.tag in ["action", "condition", "bystander"]:
                                ...
                                coord_offset = tuple(
                                    [int(i) for i in sub.attrib["coord_offset"].split()]
                                )
                                coord = Coord(
                                    layer=coord_layer,
                                    name=coord_name,
                                ...
                                if sub.tag == "bystander":
                                    ...
                                    if "flag" in sub.attrib:
                                        ...
                                        byst = Bystander(
                                            allowed_species=allowed_species, coord=coord
                                        )
                                    ...
                                    condition_action = ConditionAction(
                                        species=species, coord=coord, implicit=implicit
                                    )
                                    if sub.tag == "action":
                    ...
                    self.species_list.default_species = (
                        ...
                        if "default_species" in child.attrib
                        else ""
                    )
                    for species in child:
                        name = species.attrib["name"]
                        color = (
                            species.attrib["color"] if "color" in species.attrib else ""
                        )
                        representation = (
                            species.attrib["representation"]
                            if "representation" in species.attrib
                            else ""
                        )
                        tags = species.attrib.get("tags", "")
                        species_elem = Species(
                            name=name,
                            color=color,
    ...
    def validate_model(self):
        ...
        for layer in self.get_layers():
            for x in layer.sites:
                if len([y for y in layer.sites if x.name == y.name]) > 1:
                    raise UserWarning(
                        ('Site "%s" in Layer "%s"is not unique.') % (x.name, layer.name)
                    )
        ...
        for x in self.get_layers():
            ...
            if not variable_regex.match(layer.name):
                raise UserWarning(
                    (
                        "Lattice %s is not a valid variable name.\n"
        ...
        if len(self.get_layers()) > 1 and self.layer_list.default_layer not in [
            layer.name for layer in self.get_layers()
        ]:
            raise UserWarning(
                'Default Layer "%s" is not defined.' % self.layer_list.default_layer
            )
        ...
        for x in self.get_parameters():
            if len([y for y in self.get_parameters() if x.name == y.name]) > 1:
                raise UserWarning(
                    (
                        'The parameter "%s" has been defined two'
        ...
        if self.species_list.default_species not in [
            x.name for x in self.get_speciess()
        ]:
            raise UserWarning(
                'Default species "%s" not found.' % self.species_list.default_species
            )
        ...
        for species in self.get_speciess():
            ...
            if not variable_regex.match(species.name):
                raise UserWarning(
                    (
                        "Species %s is not a valid variable name.\n"
                        'Only letters, numerals and "_" allowed.\n'
                        "First character has to be a letter.\n"
                    )
                )
        ...
        for x in self.get_processes():
            if not variable_regex.match(x.name):
                raise UserWarning(
                    (
                        "Model %s is not a valid variable name.\n"
                        'Only letters, numerals and "_" allowed.\n'
                        "First character has to be a letter.\n"
                    )
                    % x.name
        ...
        for process in self.get_processes():
            for x in process.action_list:
                if len([y for y in process.action_list if x == y]) > 1:
                    raise UserWarning(
                        "%s of process %s is not unique!" % (x, process.name)
                    )
        ...
        for process in self.get_processes():
            for x in process.bystander_list:
                if len([y for y in process.bystander_list if x.coord == y.coord]) > 1:
                    raise UserWarning(
                        ("Found more than one bystander for %s\n" % x.coord)
                        + ("on process %s" % process.name)
                    )
                if len([y for y in process.condition_list if x.coord == y.coord]) > 0:
                    raise UserWarning(
                        "Process %s has both a condition and a bystander\n"
                        "on %s!" % (process.name, x.coord)
                    )
                if len([y for y in process.action_list if x.coord == y.coord]) > 0:
                    raise UserWarning(
                        "Process %s has an action and a bystander\n on %s!"
        ...
        for x in self.get_processes():
            for y in x.condition_list + x.action_list:
                ...
                for stripped_species in stripped_speciess:
                    if stripped_species not in species_names:
                        raise UserWarning(
                            ("Species %s used by %s in process %sis not defined")
                            % (y.species, y, x.name)
                        )
    ...
    def print_statistics(self):
        def get_name(x):
            return "_".join(x.name.split("_")[:-1])
        ...
        for process_type in sorted(names):
            nprocs = len(
                [x for x in self.get_processes() if get_name(x) == process_type]
            )
            if ml:
    ...
    def set_meta(
        self, author=None, email=None, model_name=None, model_dimension=None, debug=None
    ):
...
class Meta(object):
    ...
    def setattribute(self, attr, value):
        if attr in ["author", "email", "debug", "model_name", "model_dimension"]:
            self.add({attr: value})
...
class LayerList(FixedObject, list):
    ...
    def set_representation(self, images):
        ...
        if type(images) is list:
            ...
            raise UserWarning(
                "Data type %s of %s not understood." % (type(images), images)
            )
    ...
    def generate_coord_set(self, size=[1, 1, 1], layer_name="default", site_name=None):
        ...
        if site_name is not None and not any(
            map(
                lambda x: re.search(site_name, x),
                ["_".join(x.name.split("_")) for x in layer.sites],
            )
        ):
            raise UserWarning(
                "Layer {layer_name} has no site matching {site_name}. Please check spelling and try again.".format(
                    **locals()
                )
            )
        ...
        if site_name is None:
            return [
                self.generate_coord(
                    "%s.(%s, %s, %s).%s" % (site.name, i, j, k, layer_name)
                )
            ...
            selected_site_names = [
                site.name
                for site in layer.sites
                if re.search(site_name, "_".join(site.name.split("_")[:]))
            ]
            return [
                self.generate_coord("%s.(%s, %s, %s).%s" % (site, i, j, k, layer_name))
                for i in drange(size[0])
                for j in drange(size[1])
                for k in drange(size[2])
    ...
    def generate_coord(self, terms):
        ...
        term = terms.split(".")
        if len(term) == 3:
            coord = Coord(name=term[0], offset=eval(term[1]), layer=term[2])
        elif len(term) == 2:
            coord = Coord(name=term[0], offset=eval(term[1]), layer=self.default_layer)
        elif len(term) == 1:
            coord = Coord(name=term[0], offset=(0, 0, 0), layer=self.default_layer)
        else:
        ...
        if not sites:
            raise UserWarning(
                "No site names %s in %s found!" % (coord.name, layer.name)
            )
        else:
            site = sites[0]
        pos = site.pos
...
class Layer(FixedObject, CorrectlyNamed):
    ...
    def get_site(self, site_name):
        sites = list(filter(lambda site: site.name == site_name, self.sites))
        if not sites:
...
class Site(FixedObject):
    ...
    def __repr__(self):
        return "[SITE] {0:12s} ({1:5s}) {2:s} {3:s}".format(
            self.name, self.default_species, self.pos, self.tags
        )
...
class Coord(FixedObject):
    ...
    def __init__(self, **kwargs):
        ...
        if len(self.offset) == 1:
            ...
            self.offset = np.array([self.offset[0], self.offset[1], self.offset[2]])

        self.pos = (
            np.array([float(i) for i in kwargs["pos"].split()])
            if "pos" in kwargs
    ...
    def __gt__(self, other):
        ...
        return (
            ...
            self.offset[1],
            self.offset[2],
        ) > (other.layer, other.name, other.offset[0], other.offset[1], other.offset[2])
    ...
    def __lt__(self, other):
        return (self.layer, tuple(self.offset), self.name) < (
            other.layer,
            tuple(other.offset),
            other.name,
        )
...
class Process(FixedObject):
    ...
    def executing_coord(self):
        return sorted(self.action_list, key=lambda action: action.coord.sort_key())[
            0
        ].coord
    ...
    def evaluate_rate_expression(self, parameters={}):
        ...
        return kmos.evaluate_rate_expression(
            rate_expr=self.rate_constant, parameters=parameters
        )
...
class Bystander(FixedObject):
    ...
    def __repr__(self):
        return "[BYSTANDER] Coord:%s Allowed species: (%s)" % (
            self.coord,
            ",".join([spec for spec in self.allowed_species]),
        )
    ...
    def _shorthand(self):
        if self.coord.offset.any():
            ...
            return "%s@%s|%s" % (self.allowed_species, self.coord.name, self.flag)
...
class ConditionAction(FixedObject):
    ...
    def _shorthand(self):
        if self.coord.offset.any():
            ...
            return "%s@%s" % (self.species, self.coord.name)
...
def parse_chemical_expression(eq, process, project_tree):
    ...
    if eq.count("->") != 1:
        raise Exception(
            "Chemical expression must contain " + 'exactly one "->"\n%s' % eq
        )
    eq = re.split("->", eq)
    left, right = eq
    ...
    for term in left + right:
        if term.count("@") != 1:
            raise Exception("Each term needs to contain " + "exactly one @:\n%s" % term)
    ...
    for term in left + right:
        if term[0][0] in ["$", "^"] and term[0][1:]:
            if not filter(lambda x: x.name == term[0][1:], project_tree.get_speciess()):
                raise UserWarning("Species %s unknown " % term[0:])
        elif not filter(lambda x: x.name == term[0], project_tree.get_speciess()):
            raise UserWarning("Species %s unknown " % term[0])
    ...
    for i, term in enumerate(left + right):
        ...
        if len(coord_term) == 2:
            name = coord_term[0]
            active_layers = list(filter(lambda x: x.active, project_tree.get_layers()))
            if len(active_layers) == 1:
                ...
                if not possible_sites:
                    ...
                    raise UserWarning(
                        (
                            "Site %s is ambiguous because it"
                            + "exists on the following lattices: %s"
                        )
        ...
        if len(coord_term) == 3:
            ...
            if layer not in layer_names:
                raise UserWarning(
                    "Layer %s not known, must be one of %s" % (layer, layer_names)
                )
            else:
                layer_instance = list(
                    filter(lambda x: x.name == layer, project_tree.get_layers())
                )[0]
                site_names = [x.name for x in layer_instance.sites]
                if name not in site_names:
                    raise UserWarning(
                        "Site %s not known, must be one of %s" % (name, site_names)
                    )
    ...
    for condition in condition_list:
        if not filter(lambda x: x.coord == condition.coord, action_list):
            action_list.append(
                ConditionAction(species=default_species, coord=condition.coord)
            )
    ...
    for action in action_list:
        if not filter(
            lambda x: x.coord == action.coord, condition_list
        ) and action.species[0] not in ["^", "$"]:
            condition_list.append(
                ConditionAction(species=default_species, coord=action.coord)
            )
    ...
    for action in action_list:
        ...
        if action.species[0] == "$":
            corresponding_condition = list(
                filter(lambda x: x.coord == action.coord, condition_list)
            )
            if action.species[1:]:
                if not corresponding_condition:
                    condition_list.append(
                        ConditionAction(species=action.species[1:], coord=action.coord)
                    )
                else:
