...
# Third-party imports that don't depend on gtk
import gobject
...
# GUI imports - pygtk.require must be called before importing gtk/kiwi
import pygtk

pygtk.require("2.0")
import gtk  # noqa: E402
import kiwi.ui  # noqa: E402
from kiwi.ui.delegates import SlaveDelegate, GladeDelegate  # noqa: E402
from kiwi.ui.objectlist import ObjectTree, Column  # noqa: E402
import kiwi.ui.dialogs  # noqa: E402

# GUI forms (imported after gtk is available)
from kmos.gui.forms import (  # noqa: E402
    LayerEditor,
    MetaForm,
    InlineMessage,
    OutputForm,
...
class GTKProject(SlaveDelegate):
    ...
    def _set_treeview_hooks(self):
        ...
        self.layer_list = self.project_data.append(None, self.model_tree.layer_list)
        self.get_layers = lambda: sorted(
            self.project_data.get_descendants(self.layer_list), key=lambda x: x.name
        )
        ...
        self.add_parameter = lambda parameter: self.project_data.append(
            self.parameter_list, parameter
        )
        self.model_tree.add_parameter = self.add_parameter
        self.get_parameters = lambda: sorted(
            self.project_data.get_descendants(self.parameter_list), key=lambda x: x.name
        )
        self.model_tree.get_parameters = self.get_parameters

        # Species List
        self.species_list = self.project_data.append(None, self.model_tree.species_list)
        self.add_species = lambda species: self.project_data.append(
            self.species_list, species
        )
        self.model_tree.add_species = self.add_species
        self.get_speciess = lambda: sorted(
            self.project_data.get_descendants(self.species_list), key=lambda x: x.name
        )
        self.model_tree.get_speciess = self.get_speciess

        # Process List
        self.process_list = self.project_data.append(None, self.model_tree.process_list)
        self.add_process = lambda process: self.project_data.append(
            self.process_list, process
        )
        self.model_tree.add_process = self.add_process
        self.get_processes = lambda: sorted(
            self.project_data.get_descendants(self.process_list), key=lambda x: x.name
        )
        self.model_tree.get_processes = self.get_processes

        # Output List
        self.output_list = self.project_data.append(None, self.model_tree.output_list)
        self.add_output = lambda output: self.project_data.append(
            self.output_list, output
        )
        self.model_tree.add_output = self.add_output
        self.get_outputs = lambda: sorted(
            self.project_data.get_descendants(self.output_list), key=lambda x: x.name
        )
    ...
    def on_key_press(self, _, event):
        ...
        if gtk.gdk.keyval_name(event.keyval) == "Delete":
            if (
                isinstance(selection, Species)
                or isinstance(selection, Process)
                or isinstance(selection, Parameter)
                or isinstance(selection, Layer)
            ):
                if (
                    kiwi.ui.dialogs.yesno(
                        "Do you really want to delete '%s'?" % selection.name
                    )
                    == gtk.RESPONSE_YES
                ):
...
class UndoStack:
    ...
    def start_new_action(self, action, elem):
        ...
        self.current_action = action
        self.current_elem = elem
        self.menubar.get_widget("/MainMenuBar/MenuEdit/EditUndo").set_label(
            "Undo %s" % action
        )
    ...
    def undo(self, _):
        ...
        if self.head <= 0:
            self.menubar.get_widget("/MainMenuBar/MenuEdit/EditUndo").set_sensitive(
                False
            )
        self.menubar.get_widget("/MainMenuBar/MenuEdit/EditRedo").set_label(
...
class Editor(GladeDelegate):
        ...
        actions.add_actions(
            [
                ...
                (
                    ...
                    "<control><shift>P",
                    "Add a new process",
                    self.on_btn_add_process__clicked,
                ),
                (
                    "InsertSpecies",
                    None,
                    ...
                    "<control><shift>E",
                    "Add a new species",
                    self.on_btn_add_species__clicked,
                ),
                ("MenuHelp", None, "_Help"),
    ...
    def on_btn_add_layer__clicked(self, _button):
        ...
        if len(self.project_tree.layer_list) == 1:
            kiwi.ui.dialogs.warning(
                "Entering multi-lattice mode",
    ...
    def on_btn_help__clicked(self, _button):
        ...
        self.toast(
            ("Please refer to online help at\n%s.\n\nOr post issues at\n%s.")
            % (help_url, issues_url)
        )
...
def main():
    ...
    parser.add_option(
        "-o", "--open", dest="xml_file", help="Immediately import kmos XML file"
    )
    ...
    if hasattr(options, "export_dir") and options.export_dir:
        ...
        editor.on_btn_export_src__clicked(_button="", export_dir=options.export_dir)
        exit()
