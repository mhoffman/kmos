...
def preprocess_rate_expression(rate_constant, project_tree=None, **env):
    """
    ...
    from kmos.types import parse_condition_action, Condition

    r = dedent(rate_constant)

    # replace occupation expression with 1s and 0s.
    terms = re.findall('[^ +()]*?@[^+\n]*', r)
    if terms and project_tree is None:
        raise Exception("Need to pass project_tree to evaluate condition terms")
    term_replacements = {}
    for term in terms:
        species, coord = parse_condition_action(term.split('@'), project_tree)
        condition = Condition(species=species, coord=coord)
        term_replacements[term] = ('%%%dd' % len(term)) % int(condition in env['env'])
    ...
    if result:
        ...
        raise Exception("""The rate-constant function returned nothing.
    ...
    if escape_python:
        ...
        for line in lines:
            if re.match('^\s*%s ?' % PREFIX, line):
                ...
                python_lines += 'pass # %s' % (line.lstrip('\t\x0b\x0c\r '))
        # if the template didn't contain any meta strings
        # just return the original
        if not matched:
            return template
        try:
            exec(python_lines)
        except (SyntaxError, ValueError, IndexError, IndentationError, KeyError) :
            print(''.join([('%05d %s\n' % line)
                           for line in
